...
@section('header', 'Edit Penyimpanan')
...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Penyimpanan {{ $penyimpanan->lokasi_rak }}</h3>

        <form action="{{ route('penyimpanan.update', $penyimpanan->id_penyimpanan) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select id="id_cucian" name="id_cucian" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                        ...
                        @foreach($cucian as $c)
                            <option value="{{ $c->id_cucian }}" {{ old('id_cucian', $penyimpanan->id_cucian) == $c->id_cucian ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="lokasi_rak" name="lokasi_rak" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                        ...
                        @foreach(['A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'A7', 'A8', 'B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8'] as $rak)
                            <option value="Rak {{ $rak }}" {{ old('lokasi_rak', $penyimpanan->lokasi_rak) == "Rak $rak" ? 'selected' : '' }}>Rak {{ $rak }}</option>
                        @endforeach
                    </select>
                    @error('lokasi_rak')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_simpan" name="tanggal_simpan" value="{{ old('tanggal_simpan', $penyimpanan->tanggal_simpan->format('Y-m-d')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
